...
class ParkingConfigTest(unittest.TestCase):
    def test_repository_parking_config_loads_nested_rois(self):
        ...
        self.assertEqual(config.yolo.confidence, 0.20)
        self.assertEqual(config.yolo.car_min_bottom_ratio, 0.45)
        self.assertEqual(config.geometry.min_confirm_frames, 3)
        self.assertEqual(config.geometry.jump_reconfirm_frames, 5)
        self.assertEqual(config.geometry.max_jump_hold_frames, 3)
        self.assertEqual(config.geometry.merged_depth_reconfirm_frames, 5)
        self.assertEqual(config.geometry.merged_depth_candidate_tolerance_px, 45.0)
        self.assertEqual(config.geometry.virtual_back_y_ratio, 0.32)
        ...
        self.assertEqual(config.lidar.car_detection_roi.x_min_mm, 400.0)
        ...
        self.assertEqual(config.lidar.first_car_turn_target_y_back_mm, 0.0)
        self.assertEqual(config.lidar.first_car_confirm_scans, 3)
        self.assertEqual(config.lidar.first_car_min_points, 2)
        self.assertEqual(config.lidar.first_car_min_x_right_mm, 250.0)
        self.assertEqual(config.lidar.first_car_max_x_right_mm, 2200.0)
        self.assertEqual(config.lidar.first_car_acquire_y_back_max_mm, 350.0)
        self.assertTrue(config.planner.first_car_preemptive_turn_enabled)
        self.assertEqual(config.planner.start_forward_s, 5.0)
        self.assertEqual(config.planner.search_speed, 100)
        self.assertEqual(config.planner.initial_search_steering_trim, -10)
        self.assertEqual(config.planner.straight_steering_trim, 0)
        self.assertEqual(config.planner.gap_tracking_speed, 80)
        self.assertEqual(config.planner.position_speed, 22)
        self.assertEqual(config.planner.first_car_approach_speed, 80)
        self.assertEqual(config.lidar.max_distance_mm, 6000.0)
        self.assertEqual(config.planner.first_car_straight_s, 0.0)
        self.assertEqual(config.planner.detection_slow_hold_s, 1.5)
        self.assertTrue(config.planner.prealign_enabled)
        self.assertEqual(config.planner.prealign_speed, 60)
        ...
        self.assertEqual(config.planner.prealign_curve_grace_s, 20.0)
        ...
        self.assertEqual(config.planner.prealign_min_drive_s, 9.0)
        self.assertEqual(config.planner.prealign_confirm_frames, 3)
        self.assertEqual(config.planner.prealign_lidar_confirm_scans, 3)
        self.assertEqual(config.planner.prealign_path_hold_frames, 3)
        self.assertEqual(config.planner.prealign_yolo_min_cars, 1)
        self.assertEqual(config.planner.prealign_yolo_hold_frames, 3)
        self.assertEqual(config.planner.entry_path_loss_grace_s, 1.5)
        self.assertEqual(config.planner.lidar_y0_band_half_width_mm, 150.0)
        self.assertEqual(config.planner.lidar_y0_clear_confirm_scans, 3)
        self.assertEqual(config.planner.lidar_y0_hold_s, 3.5)
        self.assertEqual(config.planner.lidar_y0_exit_s, 4.0)
        self.assertEqual(config.lidar.safety_confirm_scans, 3)
        self.assertEqual(config.planner.lidar_evasion_speed, -24)
        self.assertEqual(config.planner.lidar_evasion_center_deadband_mm, 80.0)
        self.assertEqual(config.lidar.safety_roi.x_min_mm, -240.0)
        self.assertEqual(config.lidar.safety_roi.x_max_mm, 240.0)
        ...
        self.assertEqual(config.planner.ultrasonic_emergency_mm, 150.0)
        self.assertEqual(config.planner.ultrasonic_resume_confirm_frames, 3)
        self.assertEqual(config.planner.lidar_resume_confirm_scans, 3)
        self.assertEqual(config.planner.car_only_speed_ratio, 0.70)
        self.assertEqual(config.planner.ultrasonic_max_correction, 35)
        self.assertEqual(config.planner.ultrasonic_stale_after_s, 0.8)
        self.assertEqual(config.planner.ultrasonic_inside_max_mm, 600.0)
        ...
        self.assertEqual(config.planner.reverse_entry_unwind_max_steering, 150)
        self.assertEqual(config.planner.entry_curve_timeout_s, 16.0)
        self.assertFalse(config.planner.correction_enabled)
        self.assertEqual(config.planner.reverse_entry_speed, -40)
        self.assertEqual(config.planner.reverse_center_speed, -50)
        self.assertEqual(config.planner.reverse_aligned_speed, -30)
        ...
        self.assertEqual(config.planner.correction_reverse_speed, -60)
        ...
        self.assertEqual(config.planner.exit_speed, 60)
        ...
        self.assertEqual(tuple(config.bev.src_top_left), (0.18, 0.32))
        self.assertEqual(tuple(config.bev.src_top_right), (0.82, 0.32))
    ...
    def test_camera_option_accepts_device_index(self):
        args = parse_args(["--camera", "2"])

        self.assertTrue(args.camera_enabled)
